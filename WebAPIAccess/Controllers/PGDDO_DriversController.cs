...
{
    public class PGDDO_DriversController : ApiController
    {
        ...
        {
            List<sp_get_personnel_names_drivers> data = new List<sp_get_personnel_names_drivers>();
            con = new SqlConnection(connetionString);

            con.Open();
            SqlCommand sql = new SqlCommand("sp_get_personnel_names_drivers", con);
            sql.CommandType = CommandType.StoredProcedure;
            sql.CommandTimeout = Int32.MaxValue;
            sql.Parameters.Add(new SqlParameter("@filter", filter));
            using (SqlDataReader reader = sql.ExecuteReader())
            {
                while (reader.Read())
                {
                    sp_get_personnel_names_drivers list = new sp_get_personnel_names_drivers();
