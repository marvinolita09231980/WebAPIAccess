...
{
    public class ListOfEmployessWithDailyRateController : ApiController
    {
        ...
        {
            List<sp_get_allpersonnel_api_with_daily_rate> data = new List<sp_get_allpersonnel_api_with_daily_rate>();
            con = new SqlConnection(connetionString);

            con.Open();
            SqlCommand sql = new SqlCommand("sp_get_allpersonnel_api_with_daily_rate", con);
            ...
            using (SqlDataReader reader = sql.ExecuteReader())
            {
                while (reader.Read())
                {
                    sp_get_allpersonnel_api_with_daily_rate list = new sp_get_allpersonnel_api_with_daily_rate();
                    ...
                    list.daily_rate = reader.GetDecimal(28);
                    list.emp_rcrd_status = Convert.ToBoolean(reader.GetValue(29));
                    list.is_active = reader.GetBoolean(30);
