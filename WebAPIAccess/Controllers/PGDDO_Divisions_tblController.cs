...
namespace WebAPIAccess.Controllers
{
    public class PGDDO_Divisions_tblController : ApiController
    {
        ...
        {
            List<sp_get_division_tbl_api> data = new List<sp_get_division_tbl_api>();
            con = new SqlConnection(connetionString);

            con.Open();
            SqlCommand sql = new SqlCommand("sp_get_division_tbl_api", con);
            ...
            using (SqlDataReader reader = sql.ExecuteReader())
            {
                while (reader.Read())
                {
                    sp_get_division_tbl_api list = new sp_get_division_tbl_api();
                    list.effective_date        =  reader.GetString(0);
                    ...
                    list.sort_order_div        =  (int)reader.GetSqlInt32(7);
